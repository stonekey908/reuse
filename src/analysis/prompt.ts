...
- **Multi-member clusters MUST span at least two distinct projects.** A "cluster" of 4 codeview submodules is not a cross-project capability cluster — it's intra-project module structure that belongs in the project's own design docs, not in this analysis. If all members come from one project, either (a) merge with related patterns from other projects to form a real cross-project cluster, or (b) demote each member to a standalone. Refuse to emit single-project-only clusters. Before emitting any cluster, count the distinct project names among its members — if there is only one, the cluster is invalid.
- **Cluster by CAPABILITY, never by MECHANISM.** This is the most common failure mode and the bar for clustering. Two patterns that share the same MECHANISM but solve different user-facing PROBLEMS do NOT cluster:
  - React Context as a mechanism: a toast notification system, a tested-context-providers test rig, and a context-per-domain-architecture all USE Context — but the capabilities are "transient UX feedback", "test-discipline coverage", and "scaling state across many domains" respectively. These are three standalones, not one cluster.
  - "Wraps a modal": a modal-shell-primitive (reusable layout infrastructure) and a barcode-scanner-modal (concrete feature consumer) are different ABSTRACTION LEVELS. Primitives cluster with primitives. Feature consumers cluster with feature consumers. They do NOT cluster across the abstraction-level gap.
  - "Calls an AI API": a multi-provider routing pattern and a heavily-tested single-provider service share the AI mechanism but the capabilities are "abstracting which provider runs" vs "test rigor for prompt assembly". The latter is a discipline pattern, not a routing pattern. Different clusters or standalones.
- **Discipline patterns are not capability patterns.** Patterns about testing, error handling, performance monitoring, observability, or other engineering discipline applied to a capability are NOT clustered with patterns that implement that capability. A "heavily-tested Gemini service" pattern is about TEST COVERAGE DISCIPLINE; it stands alone (or clusters with other test-discipline patterns from other projects), not with multi-provider routing.
- **Don't cluster across abstraction levels.** Within a domain, a reusable infrastructure primitive and a concrete consumer of that primitive belong in different clusters — even if both touch the same surface. "Modal layout shells" (primitives) and "barcode scanner modal" (consumer) are not the same capability.
